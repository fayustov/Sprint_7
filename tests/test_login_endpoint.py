...
class TestLoginEndpoint:
    ...
    login_endpoint_url = f"{Helpers.BASE_URL}/api/v1/courier/login"

    @allure.title('Проверка требований: «курьер может авторизоваться» и «успешный запрос возвращает id»')
    @allure.link(f'{Helpers.BASE_URL}/docs/#api-Courier-Login',
                 name='Ссылка на документацию эндпоинта Логин курьера в системе')
    def test_courier_login_positive_return_200(self):

        response = requests.post(TestLoginEndpoint.login_endpoint_url, data=Helpers.correct_test_user_payload)
    ...
    def test_courier_login_with_incorrect_data_return_404(self):
        ...
        response = requests.post(TestLoginEndpoint.login_endpoint_url, data=Helpers.incorrect_test_user_payload)
    ...
    def test_login_without_required_fild(self, field):
        ...
        response = requests.post(TestLoginEndpoint.login_endpoint_url, data=payload, timeout=1)
